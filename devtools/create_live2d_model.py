...
def normalize_model(model, shipgirl):
    ...
    for part in PART_NAMES:
        ...
        part_data.setdefault("idle", [0, None])
        part_data.setdefault("walk", [0, None])
    return model
...
def draw_status(surface, font_registry, shipgirl, selected_part, part_data, preview_animation, panel_x, panel_y):
    ...
    lines = [
        ...
        f"Preview: {'idle' if preview_animation == Live2D.IDLE_ANIMATION else 'walk'}",
        f"Idle: amp {idle[0]}, wave {idle[1]}",
        f"Walk: amp {walk[0]}, wave {walk[1]}",
        "",
        "Left/Right or Tab: layer",
        "Space: preview idle/walk",
        "Q/E: idle amplitude -/+5",
        "W: idle wave, R: clear idle",
        "A/D: walk amplitude -/+5",
        "S: walk wave, F: clear walk",
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    preview_animation = (
                        ...
                        if preview_animation == Live2D.IDLE_ANIMATION
                        else Live2D.IDLE_ANIMATION
                    )
                    live2d.set_animation(preview_animation)
                elif event.key == pygame.K_e:
                    change_amplitude(selected_part_data["idle"], 5)
                    changed = True
                elif event.key == pygame.K_q:
                    change_amplitude(selected_part_data["idle"], -5)
                    changed = True
                elif event.key == pygame.K_w:
                    cycle_wave(selected_part_data["idle"], 1)
                    changed = True
                elif event.key == pygame.K_r:
                    selected_part_data["idle"] = [0, None]
                    changed = True
                elif event.key == pygame.K_d:
                    change_amplitude(selected_part_data["walk"], 5)
                    changed = True
                    ...
                    cycle_wave(selected_part_data["walk"], 1)
                    changed = True
                elif event.key == pygame.K_f:
                    selected_part_data["walk"] = [0, None]
                    changed = True
