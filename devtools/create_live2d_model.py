...
def default_model(shipgirl):
    ...
    return migrate_live2d_models.migrate_model(model)


def load_or_create_model(shipgirl):
    model_path = model_path_for(shipgirl)
    if os.path.exists(model_path):
        migrate_live2d_models.migrate_file(Path(model_path))
...
def draw_status(surface, font, shipgirl, selected_part, part_data, panel_x, panel_y):
    lines = [
        f"Shipgirl: {shipgirl}",
        ...
        "Click enlarged layer to set pivot",
...
def main():
    ...
    live2d.set_animation(Live2D.IDLE_ANIMATION)
    selected_index = 0
